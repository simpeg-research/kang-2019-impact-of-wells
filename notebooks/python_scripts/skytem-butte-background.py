...
inds_layer_near = (
    ...
    (np.logical_and(mesh.gridCC[:,1]>-605, mesh.gridCC[:,1]<605))
)   

from scipy.interpolate import interp1d
import pandas as pd
df_rho = pd.read_csv("./1d-avg-resistivity.csv")
f_sigma = interp1d(df_rho['z'].values, 1./df_rho['rho'].values, fill_value='extrapolate')

sigma[~inds_air] = f_sigma(mesh.gridCC[~inds_air,2])
