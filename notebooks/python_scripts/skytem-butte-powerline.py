...
result_dir = "../skytem_butte_powerline/"
...
# layer_ind_1 = np.logical_and(mesh.gridCC[:,2]>-30, mesh.gridCC[:,2]<0.) & (levels>5.)
layer_ind_1 = np.logical_and(mesh.gridCC[:,2]>-10, mesh.gridCC[:,2]<0.) & (levels>5.)
layer_ind_2 = np.logical_and(mesh.gridCC[:,2]>-170, mesh.gridCC[:,2]<-120) & (levels>5.)

sigma[layer_ind_1] = 1./50.
sigma[layer_ind_2] = 1./10.
...
np.save(result_dir+'data-30ohmm', data)
