...
octreeLevels = [
    ...
    [0, 0, 0, 0, 0, 4, 4, 10],
    [0, 0, 0, 0, 0, 0, 4, 4, 10],
    [0, 0, 0, 0, 0, 0, 0, 4, 4, 10],
    [0, 0, 0, 0, 0, 0, 0, 0, 4, 4, 10]
]
maxLevels = [11, 11, 11, 11, 11, 11]
result_dir = "../accuracy/"

for i_hs, h_temp in enumerate(hs):

    h_temp = hs[i_hs]
    maxLevel = maxLevels[i_hs]

    h = [h_temp, h_temp, h_temp]

    octreeLevel = octreeLevels[i_hs]
    ...
    # print (np.argwhere(cell_size==4.)[0][0])
    ...
    np.save(result_dir+'data' + str(i_hs), data)
